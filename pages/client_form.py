...
def client_form(page: ft.Page):
    page.client_storage.set("edit_client_id", 0)
    ...
    def submit_client(_):
        ...
        try:
            ...
            print("Client Response",response.json()["detail"])
    ...
    def populate_client_fields(client_array):
        name_field.value = client_array.get("clientname", " ")
        lastname_field.value = client_array.get("clientlastname", " ")
        phone_field.value = client_array.get("clientphone", " ")
        email_field.value = client_array.get("clientemail", " ")
        address_field.value = client_array.get("clientaddress", " ")
        status_switch.value = client_array.get("clientstatus", " ")
        elig_switch.value = client_array.get("iselegible", " ")
        type_dropdown.value = client_array.get("idtypeclient", " ")
        risk_dropdown.value = client_array.get("idriskclient", " ")
        page.client_storage.set("edit_client_id", str(client_array.get("idclient", 0)))
        page.update()
    ...
    return ft.Row([
        ...
        ft.Container(
            ...
            padding=20,
            content=ft.Column([
                ft.Text("Manage Clients", size=20, weight=ft.FontWeight.BOLD),
                name_field,
                lastname_field,
                email_field,
                address_field,
